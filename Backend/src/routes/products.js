...
// --> Now logs stock movement if stock_quantity changes (IN/OUT), transaction-safe
router.put("/edit/:id", async (req, res) => {
  ...
  try {
    ...
    // Start transaction to safely update product and possibly log stock movement
    await connection.beginTransaction();

    // Lock the product row for update and ensure tenant scope
    ...
    const oldStock = Number(productRows[0].stock_quantity || 0);
    ...
    // If stock changed, log it in stock_movements
    if (stockChange !== 0) {
...
// ==================== OTHER ROUTES (unchanged except tenant-scope fixes) ====================
...
// Delete product (tenant-specific) - consider tenant scope for invoice_items check too
router.delete("/delete/:id", async (req, res) => {
  const productId = req.params.id;

  try {
    const [checkRef] = await db.execute(
      "SELECT COUNT(*) AS count FROM invoice_items WHERE product_id = ? AND tenant_id = ?",
      [productId, req.user.tenant_id]
    );
...
// Fixed: include tenant_id in stock_movements insert and use updated_by from req.user
router.post("/update-stock", async (req, res) => {
  ...
  try {
    ...
    const oldStock = Number(rows[0].stock_quantity || 0);
    ...
    // Only insert a movement if there's an actual change
    if (changeType) {
      const qtyChanged = Math.abs(stockChange);
      const updatedBy =
        updated_by ||
...
// Fetch product by barcode (tenant-specific)
